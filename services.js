...
function getUsers() {
	...
	document.getElementById('user_dropdown').innerHTML =
		'<li class="divider" tabindex="-1"></li><li><a class="grey lighten-3 black-text" onclick="createUserModalInstance.open()">Create New User</a></li>';
...
function getTaskTemplate(cursor) {
	...
	div12.textContent = cursor_value.title;
			...
			if (cursor && !s) {
				if (cursor.value.id === listFilterID) {
					document.getElementById(
						'main_heading_1'
					).textContent = `My Day - List: ${cursor.value.title}`;
					document.getElementById(
						'main_heading_2'
					).textContent = `All Tasks - List: ${cursor.value.title}`;
					s = true;
				}
				cursor.continue();
...
function updateTaskDetails() {
	...
	if (
		...
		document.getElementById('update_task_title').value == '' ||
		document.getElementById('update_task_due_date').value == '' ||
		document.getElementById('update_task_reminder_date').value == '' ||
		document.getElementById('update_task_reminder_time').value == ''
	) {
		document.getElementById('update_task_form_errors').textContent =
